...
function erase_pencil(){
  ...
  for(let i = 0; i<pencil.length ; i++){ // 그려진 그림의 획 만큼 반복합니다.
    for(let j = 0; j<pencil[i].length; j++){ // 각 획에 저장된 좌표 갯수만큼 반복합니다.
...
function draw_mode(){
  ...
  else if(cur_pinger_num == 0){ 
    //주먹을 쥔 경우

    pencil = []; // 저장된 좌표값들을 모두 초기화 시킵니다.
...
//draw에서 지속적으로 호출되며 pencil변수에 저장된 좌표로 라인을 그려줍니다.
function show_draw(){
  push()
  ...
  for(let i = 0; i<pencil.length ; i++){ // 획의 갯수 만큼 반복합니다.
    for(let j = 1; j<pencil[i].length; j++){ // 현재 획의 저장된 좌표의 갯수-1만큼 저장합니다.
      line( pencil[i][j-1].x, pencil[i][j-1].y,  pencil[i][j].x,  pencil[i][j].y  );//저장된 좌표를 그립니다.
    }
  }
